...
<template>
  <div v-if="hasItems" class="m-10">
    ...
    <div class="my-10 md:grid md:grid-cols-3 md:gap-8">
      <ul role="list" class="divide-y pl-0 divide-gray-200 md:col-span-2 border-t">
        <li v-for="cartItem in cartItems" :key="cartItem.id" class="flex py-6">
          <CheckoutCartItem :cart-item="cartItem" />
        </li>
      </ul>

      <aside class="md:col-span-1 pb-4 px-4 bg-gray-50 rounded dark:bg-gray-800">
        ...
        <NuxtLink
          class="flex items-center justify-center rounded-full px-6 py-3 text-base font-medium text-white shadow-sm bg-black hover:bg-brand-dark"
          data-testid="cart-checkout-link" :to="formatLink(`/checkout`)">
          {{ $t("cart.checkout") }}
        </NuxtLink>
      </aside>
    </div>
