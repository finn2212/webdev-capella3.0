...
<template>
  <div class="m-10">
    <SharedModal :controller="loginModalController">
      <AccountLoginForm @close="loginModalController.close" @success="loginModalController.close" />
    </SharedModal>
    <SharedModal :controller="addAddressModalController">
      <SharedAccountAddressForm @success="
      refreshAddresses();
    addAddressModalController.close();
    " />
    </SharedModal>
    <div v-if="isCheckoutAvailable || isCartLoading" class="checkout-inner" :class="{
      ...
      <div class="md:grid md:grid-cols-2 md:gap-6">
        <div class="md:col-span-1">
          <div class="grid gap-4 shadow px-4 py-5 bg-white sm:p-6 mb-8">
            ...
            <form v-if="!isUserSession" id="checkout-billing-address" class="grid gap-8" name="checkout-billing-address"
              method="post" @submit.prevent="invokeSubmit">
              <div v-if="registerErrors.length" class="bg-red-200 border-l-4 border-red-500 text-red-700 p-4"
                role="alert">
              ...
              <div class="text-sm">
                ...
                <a href="#" class="whitespace-nowrap font-medium text-brand-primary hover:text-brand-dark"
                  data-testid="checkout-sign-in-link" @click="loginModalController.open">
                  {{ $t("checkout.signIn") }}
                </a>
              ...
              <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6">
                  <label for="salutation" class="block text-sm font-medium text-gray-700">{{ $t("form.salutation")
                    }}</label>
                  <select id="salutation" v-model="state.salutationId" required name="salutation" autocomplete="on"
                    ...
                    data-testid="checkout-pi-salutation-select" @blur="$v.salutationId.$touch()">
                  ...
                  <span v-if="$v.salutationId.$error"
                    class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
                    {{ $v.salutationId.$errors[0].$message }}
                  </span>
                </div>
                <div class="col-span-6 sm:col-span-3">
                  <label for="first-name" class="block text-sm font-medium text-gray-700">{{ $t("form.firstName")
                    }}</label>
                  <input id="first-name" v-model="state.firstName" type="text" required name="first-name"
                    :placeholder="$t('form.firstNamePlaceholder')"
                    class="mt-1 block w-full p-2.5 border border-gray-300 text-gray-900 text-sm rounded-md shadow-sm focus:ring-brand-light focus:border-brand-light"
                    data-testid="checkout-pi-first-name-input" @blur="$v.firstName.$touch()" />
                ...
                <div class="col-span-6 sm:col-span-3">
                  <label for="last-name" class="block text-sm font-medium text-gray-700">{{ $t("form.lastName")
                    }}</label>
                  <input id="last-name" v-model="state.lastName" type="text" required name="last-name"
                    :placeholder="$t('form.lastNamePlaceholder')"
                    class="mt-1 block w-full p-2.5 border border-gray-300 text-gray-900 text-sm rounded-md shadow-sm focus:ring-brand-light focus:border-brand-light"
                    data-testid="checkout-pi-last-name-input" @blur="$v.lastName.$touch()" />
                ...
                <div class="col-span-6">
                  <div class="flex items-center">
                    ...
                    <label for="create-account" class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">{{
      $t("checkout.notCreateAccount") }}</label>
                  </div>
                </div>

                <div class="col-span-6 sm:col-span-3">
                  <label for="email-address" class="block text-sm font-medium text-gray-700">{{ $t("form.email")
                    }}</label>
                  <input id="email-address" v-model="state.email" type="email" required name="email-address"
                    :placeholder="$t('form.emailPlaceholder')" autocomplete="off"
                    class="mt-1 block w-full p-2.5 border border-gray-300 text-gray-900 text-sm rounded-md shadow-sm focus:ring-brand-light focus:border-brand-light"
                    data-testid="checkout-pi-email-input" @blur="$v.email.$touch()" />
                ...
                <div class="col-span-6 sm:col-span-3">
                  <div v-if="!state.guest">
                    <label for="password" class="block text-sm font-medium text-gray-700">{{ $t("form.password")
                      }}</label>
                    <input id="password" v-model="state.password" autocomplete="off" type="password" name="password"
      ...
      $t("form.streetAddress")
                  ...
                  <input id="street-address" v-model="state.billingAddress.street" type="text" required
                    name="street-address" :placeholder="$t('form.streetPlaceholder')" autocomplete="street-address"
                    class="mt-1 block w-full p-2.5 border border-gray-300 text-gray-900 text-sm rounded-md shadow-sm focus:ring-brand-light focus:border-brand-light"
                    data-testid="checkout-pi-street-address-input" @blur="$v.billingAddress.street.$touch()" />
                  <span v-if="$v.billingAddress.street.$error"
                    class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
                ...
                <div class="col-span-6 sm:col-span-3">
                  <label for="postal-code" class="block text-sm font-medium text-gray-700">{{ $t("form.postalCode")
                    }}</label>
                  <input id="postal-code" v-model="state.billingAddress.zipcode" type="text" required name="postal-code"
                    :placeholder="$t('form.postalCodePlaceholder')" autocomplete="postal-code"
                    class="mt-1 block w-full p-2.5 border border-gray-300 text-gray-900 text-sm rounded-md shadow-sm focus:ring-brand-light focus:border-brand-light"
                    data-testid="checkout-pi-zip-code-input" @blur="$v.billingAddress.zipcode.$touch()" />
                  <span v-if="$v.billingAddress.zipcode.$error"
                    class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
                    {{ $v.billingAddress.zipcode.$errors[0].$message }}
                  </span>
                </div>
                <div class="col-span-6 sm:col-span-3">
                  <label for="city" class="block text-sm font-medium text-gray-700">{{ $t("form.city") }}</label>
                  <input id="city" v-model="state.billingAddress.city" type="text" required name="city"
                    :placeholder="$t('form.cityPlaceholder')" autocomplete="address-level2"
                    class="mt-1 block w-full p-2.5 border border-gray-300 text-gray-900 text-sm rounded-md shadow-sm focus:ring-brand-light focus:border-brand-light"
                    data-testid="checkout-pi-city-input" @blur="$v.billingAddress.city.$touch()" />
                  <span v-if="$v.billingAddress.city.$error"
                    class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
                ...
                <SharedCountryStateInput v-model:countryId="state.billingAddress.countryId"
                  ...
                  :state-id-validation="$v.billingAddress.countryStateId" class="col-span-6" />
              </div>
              <button type="submit"
                class="flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-full text-white bg-black hover:bg-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-primary"
      ...
      $t("checkout.guest") }}
              ...
              <span v-else>
                ...
                <a href="#" class="text-brand-primary font-bold hover:text-brand-dark" data-testid="checkout-logout"
                  aria-label="click here to log out" @click="invokeLogout">{{ $t("checkout.logOut") }}</a>
              </span>

            </div>
          </div>
          <fieldset v-if="!isVirtualCart" class="grid gap-4 shadow px-4 py-5 bg-white sm:p-6 mb-8">
            <legend class="pt-5">
              <h3 class="text-lg font-medium text-gray-900 m-0">
            ...
            <div v-for="singleShippingMethod in shippingMethods" v-else :key="singleShippingMethod.id"
              class="flex items-center w-full">
              <input :id="singleShippingMethod.id" v-model="selectedShippingMethod" :value="singleShippingMethod.id"
                name="shipping-method" type="radio" class="focus:ring-brand-primary h-4 w-4 border-gray-300"
                :data-testid="`checkout-shipping-method-${singleShippingMethod.id}`" />
              <label :for="singleShippingMethod.id" :class="{ 'animate-pulse': isLoading[singleShippingMethod.id] }"
      ...
      getShippingMethodDeliveryTime(singleShippingMethod)
                    ...
                    <span v-if="singleShippingMethod.translated?.description"
                      class="italic text-sm text-gray-500 block mt-3">
                      {{ singleShippingMethod.translated.description }}</span>
                  </div>
                  <div v-if="singleShippingMethod.media?.url">
                    <img loading="lazy" :src="singleShippingMethod.media.url" alt="payment-image" class="max-w-14" />
                  </div>
                </div>
              </label>
            </div>
          ...
          <fieldset class="grid gap-4 shadow px-4 py-5 bg-white sm:p-6">
            ...
            <div v-for="singlePaymentMethod in paymentMethods" v-else :key="singlePaymentMethod.id"
              class="flex items-center">
              <input :id="singlePaymentMethod.id" v-model="selectedPaymentMethod" :value="singlePaymentMethod.id"
                name="payment-method" type="radio" class="focus:ring-brand-primary h-4 w-4 border-gray-300"
                :data-testid="`checkout-payment-method-${singlePaymentMethod.id}`" />
              <label :for="singlePaymentMethod.id" :class="{ 'animate-pulse': isLoading[singlePaymentMethod.id] }"
                class="ml-2 block text-sm font-medium text-gray-700 w-full">
                <div class="flex justify-between">
                  <div>
                    <span>
                    ...
                    <span v-if="singlePaymentMethod.translated?.description" class="italic text-sm text-gray-500 block">
                      {{ singlePaymentMethod.translated.description }}</span>
                  </div>
                  <div v-if="singlePaymentMethod.media?.url">
                    <img loading="lazy" :src="singlePaymentMethod.media.url"
          ...
          <fieldset class="grid gap-4 shadow px-4 py-5 bg-white sm:p-6">
            ...
            <template v-if="!isLoading['addresses']">
              <div v-for="address in customerAddresses" :key="address.id" class="flex mb-3">
                <input :id="`billing-${address.id}`" v-model="selectedBillingAddress" :value="address.id"
                  name="billing-address" type="radio" class="focus:ring-brand-primary h-4 w-4 border-gray-300"
                  :data-testid="`checkout-billing-address-${address.id}`" />
                <label :for="`billing-${address.id}`" class="ml-2 field-label">
                  <AccountAddressCard :key="address.id" :address="address" :countries="getCountries"
                    :salutations="getSalutations" :can-set-default="false" @success="refreshAddresses()" />
                </label>
              </div>
            </template>
            <button type="button" data-testid="checkout-add-new-billing-address-button"
              class="flex font-medium text-brand-dark bg-transparent" @click="addAddressModalController.open">
            ...
            <template v-if="!isVirtualCart && !isLoading['addresses']">
              <label for="customShipping" class="field-label">
                <input id="customShipping" v-model="state.customShipping"
                  data-testid="checkout-custom-shipping-address-checkbox" name="privacy" type="checkbox"
              ...
              <div v-if="state.customShipping">
                <div v-for="address in customerAddresses" :key="address.id" class="flex mb-3">
                  <input :id="`shipping-${address.id}`" v-model="selectedShippingAddress" :value="address.id"
                    name="shipping-address" type="radio" class="focus:ring-brand-primary h-4 w-4 border-gray-300"
                    :data-testid="`checkout-shipping-address-${address.id}`" />
                  <label :for="`shipping-${address.id}`" :class="{
      'animate-pulse': isLoading[`shipping-${address.id}`],
                ...
                <button type="button" data-testid="checkout-add-new-shipping-address-button"
                  class="flex font-medium text-brand-dark bg-transparent" @click="addAddressModalController.open">
          ...
          <fieldset ref="termsBox" class="grid gap-4 shadow px-4 py-5 bg-white sm:p-6" data-testid="checkout-terms-box">
            <legend class="pt-5">
              <h3 class="text-lg font-medium text-gray-900 m-0">
            ...
            <div class="flex items-center" data-testid="checkout-t&c-tos">
              <input id="tos" v-model="terms.tos" :value="terms.tos" name="tos" type="checkbox"
                class="focus:ring-brand-primary h-4 w-4 border-gray-300 shrink-0"
                data-testid="checkout-t&c-checkbox-tos" />
              <label for="tos" class="ml-2 block text-sm font-medium text-gray-700"
                :class="{ 'text-red': !termsSelected && placeOrderTriggered }">
                Ich habe die
                <nuxt-link to="/agb" class="text-brand-primary hover:text-brand-primary-dark">Allgemeinen
                  Geschäftsbedingungen</nuxt-link>
                gelesen und akzeptiert.
              </label>
            </div>

            <div v-if="isVirtualCart" class="flex items-center" data-testid="checkout-t&c-revocation">
              <input id="revocation" v-model="terms.revocation" :value="terms.revocation" name="revocation"
                type="checkbox" class="focus:ring-brand-primary h-4 w-4 border-gray-300 shrink-0"
                data-testid="checkout-t&c-checkbox-revocation" />
              <label for="revocation" class="ml-2 block text-sm font-medium text-gray-700"
                :class="{ 'text-red': !termsSelected && placeOrderTriggered }">
      ...
      $t("checkout.loginRequired")
                ...
                <button :disabled="!isUserSession" type="button" :class="{
      grayscale: !isUserSession,
      'opacity-50 cursor-not-allowed hover:bg-brand-primary':
        !isUserSession,
      'animate-pulse': isLoading['placeOrder'],
    }" class="w-full flex justify-center py-2 px-4 border border-transparent font-medium rounded-full text-white bg-black focus:outline-none focus:ring-2 focus:ring-brand-primary"
                  data-testid="checkout-place-order-button" @click="placeOrder">
    ...
    <div v-else class="text-center">
      ...
      <NuxtLink
        ...
        :to="formatLink(`/`)" data-testid="checkout-go-home-link">
