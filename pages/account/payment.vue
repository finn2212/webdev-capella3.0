...
<template>
  <div class="container mx-auto my-8">
    <fieldset class="mt-6">
      ...
      <form class="mt-4 space-y-6" data-testid="account-payment" @submit.prevent="invokeSave">
        ...
        <div v-else class="mt-4 space-y-4">
          <div v-for="paymentMethod in paymentMethods" :key="paymentMethod.id" class="flex items-center">
            <input :id="paymentMethod.id" v-model="formData.paymentMethod" :value="paymentMethod.id"
              :checked="selectedPaymentMethod?.id === paymentMethod.id" name="payment-method" type="radio"
          ...
          <button
            class="group relative justify-center py-2 px-4 my-4 border border-transparent text-sm font-medium rounded-full text-white bg-black hover:bg-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-light"
            type="submit" data-testid="account-payment-submit-button">
            {{ $t("account.save") }}
          </button>
        </div>
      </form>
