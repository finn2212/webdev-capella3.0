...
<template>
  <div class="max-w-screen-xl mx-auto px-6 sm:px-4">
    <div v-if="showDoubleOptInBox" ref="doubleOptInBox"
      class="bg-green-100 border-t border-b border-green-500 text-green-700 px-4 py-3 mb-4">
      {{ $t("account.messages.signUpSuccess") }}
    </div>
    <form class="w-full relative" data-testid="registration-form" @submit.prevent="invokeSubmit">
      ...
      $v.salutationId.$error
    ...
    ]" data-testid="registration-salutation-select" @blur="$v.salutationId.$touch()">
        ...
        <div class="col-span-12 md:col-span-4">
          ...
          <input id="first-name" v-model="state.firstName" name="first-name" type="text" autocomplete="first-name"
            class="appearance-none relative block w-full px-3 py-2 border placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:z-10 sm:text-sm"
            :class="[
      $v.firstName.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]" :placeholder="$t('form.firstNamePlaceholder')" data-testid="registration-first-name-input"
            @blur="$v.firstName.$touch()" />
          <span v-if="$v.firstName.$error" class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
      ...
      $v.lastName.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]" :placeholder="$t('form.lastNamePlaceholder')" data-testid="registration-last-name-input"
            @blur="$v.lastName.$touch()" />
          <span v-if="$v.lastName.$error" class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
        ...
        <div class="col-span-12 md:col-span-6">
          ...
          <input id="email-address" v-model="state.email" name="email" type="email" autocomplete="email" :class="[
      $v.email.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]"
            class="appearance-none relative block w-full px-3 py-2 border placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:z-10 sm:text-sm"
            :placeholder="$t('form.emailPlaceholder')" data-testid="registration-email-input"
            @blur="$v.email.$touch()" />
      ...
      $v.password.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]" :placeholder="$t('form.passwordPlaceholder')" data-testid="registration-password-input"
            @blur="$v.password.$touch()" />
          <span v-if="$v.password.$error" class="pt-1 text-sm text-red-600 focus:ring-brand-primary border-gray-300">
      ...
      $v.billingAddress.street.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]" :placeholder="$t('form.streetPlaceholder')" data-testid="registration-street-input"
      ...
      $v.billingAddress.zipcode.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]" :placeholder="$t('form.postalCodePlaceholder')" data-testid="registration-zipcode-input"
      ...
      $v.billingAddress.city.$error
        ? 'border-red-600 focus:border-red-600'
        : 'border-gray-300 focus:border-indigo-500',
    ]" :placeholder="$t('form.cityPlaceholder')" data-testid="registration-city-input"
        ...
        <SharedCountryStateInput v-model:countryId="state.billingAddress.countryId"
          v-model:stateId="state.billingAddress.countryStateId" :country-id-validation="$v.billingAddress.countryId"
          :state-id-validation="$v.billingAddress.countryStateId" class="col-span-12 md:col-span-4" />
      </div>
      <div class="mb-5 text-right">
        <button
          class="group relative py-2 px-4 border border-transparent text-sm font-medium rounded-full text-white bg-black hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-75 w-full md:w-auto"
          type="submit" data-testid="registration-submit-button">
          {{ $t("form.submit") }}
        </button>
      </div>
    </form>
