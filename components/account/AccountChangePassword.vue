...
<template>
  <div class="space-y-8">
    ...
    <form class="mt-8 space-y-6" data-testid="account-change-password-form" @submit.prevent="invokeChange">
      <div v-if="userErrorMessages.length" class="text-red-600 focus:ring-brand-primary border-gray-300 rounded">
        {{ userErrorMessages }}
      </div>

      <div class="mt-4 space-y-4 lg:mt-5 md:space-y-5">
        <div>
          <label for="current-password" class="block mb-2 text-sm font-medium text-gray-500 dark:text-white">{{
            $t("changePassword.form.currentPassword") }}</label>
          <input id="current-password" v-model="state.password.currentPassword" name="curent-password" type="password"
            ...
            placeholder="••••••••" data-testid="account-change-current-password-input" :disabled="loadingData" />
          <span v-if="$v.password.currentPassword.$error"
            class="text-red-600 focus:ring-brand-primary border-gray-300 rounded">
            {{ $v.password.currentPassword.$errors[0].$message }}
          </span>
        </div>
        <div>
          <label for="new-password" class="block mb-2 text-sm font-medium text-gray-500 dark:text-white">{{
          ...
          <input id="new-password" v-model="state.password.newPassword" name="new-password" type="password"
            ...
            class="appearance-none rounded-md shadow-sm relative block w-full px-3 py-2 border border-gray-300 text-gray-900 focus:outline-none focus:ring-brand-primary focus:border-brand-primary focus:z-10 sm:text-sm"
            placeholder="••••••••" data-testid="account-change-new-password-input" :disabled="loadingData" />
        ...
        <div>
          <label for="confirm-password" class="block mb-2 text-sm font-medium text-gray-500 dark:text-white">{{
            $t("changePassword.form.resetPassword") }}</label>
          <input id="confirm-password" v-model="state.password.newPasswordConfirm" name="confirm-password" type="password"
            autocomplete="off" required
            class="appearance-none rounded-md shadow-sm relative block w-full px-3 py-2 border border-gray-300 text-gray-900 focus:outline-none focus:ring-brand-primary focus:border-brand-primary focus:z-10 sm:text-sm"
            placeholder="••••••••" data-testid="account-change-confirm-password-input" :disabled="loadingData" />
      ...
      <div>
        <button
          ...
          type="submit" data-testid="account-change-current-submit-button">
