  ...
  <Teleport to="body">
    <Transition enter-active-class="transition ease-out duration-200 transform" enter-from-class="opacity-0"
      ...
      <div v-show="isOpen" class="fixed z-10 inset-0 overflow-y-auto bg-black bg-opacity-50">
        <div class="flex items-center justify-center min-h-screen lg:-mt-3% text-center">
          <Transition enter-active-class="transition ease-out duration-500 transform "
            ...
            <div v-if="isOpen" id="modal-content" ref="modalContentElement"
              class="bg-white rounded-lg text-left overflow-hidden shadow-xl p-8" role="dialog" aria-modal="true"
              aria-labelledby="modal-headline" style="margin-top: 200px;">
              <slot></slot>
            </div>
          </Transition>
        </div>
