...
const addToCartProxy = async () => {
  ...
  pushSuccess(`${props.product?.translated?.name} wurde dem Warenkorb hinzugefügt`);
};
...
<template>
  <div class="flex flex-row mt-10">
    <div class="basis-1/4 relative -top-6">
      ...
      <input id="qty" v-model="quantity" type="number" :min="product.minPurchase || 1"
        :max="product.calculatedMaxPurchase" :step="product.purchaseSteps || 1"
    ...
    <div class="basis-3/4 ml-4">
      <button
        ...
        data-testid="add-to-cart-button" :disabled="loading" @click="addToCartProxy">
        🛍 {{ $t("product.addToCart") }}
      </button>
    </div>
  </div>
